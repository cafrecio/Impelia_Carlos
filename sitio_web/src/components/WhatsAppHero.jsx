...
export default function WhatsAppHero() {
  ...
  return (
    <div className="relative w-full h-[600px] max-w-[500px] sm:max-w-[550px] mx-auto animate-in fade-in duration-700">
      
      {/* 2. DYNAMIC SPREADSHEET PREVIEW (Background layer - bottom right) */}
      <div className="absolute bottom-0 right-0 w-[92%] sm:w-[95%] h-[320px] bg-white rounded-2xl border border-slate-200/80 shadow-2xl overflow-hidden flex flex-col z-10 transition-all duration-500">
        ...
        <div className="bg-emerald-800 text-white px-4 py-3.5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2 pl-[240px] sm:pl-[285px]"> {/* Offset so title is visible to the right of the phone */}
            ...
            <div className="text-left leading-none">
              <p className="font-display font-extrabold text-xs sm:text-sm leading-none">Planilla de Reservas</p>
              <p className="text-[9px] sm:text-[10px] text-emerald-250 mt-1.5 font-semibold">Sincronizado vía API</p>
            </div>
          </div>
          <span className="text-[8px] sm:text-[9px] bg-emerald-700 px-2 py-0.5 rounded font-bold text-emerald-100 flex items-center gap-0.5 shrink-0">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-350 animate-ping"></span>
            En vivo
          </span>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase font-bold tracking-wider text-[8px] sm:text-[9.5px] sticky top-0 z-10 shrink-0">
                ...
                <th className="px-3 py-2.5 border-r border-slate-100">Total</th>
                <th className="px-3 py-2.5">Estado</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-[10px] sm:text-[12px]">
              {sheetData.map((row, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 border-r border-slate-100 font-bold text-slate-950 text-left">{row.amount}</td>
                    <td className="px-3 py-3 text-left">
                      <span className="inline-flex items-center gap-0.5 text-[8.5px] sm:text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">
